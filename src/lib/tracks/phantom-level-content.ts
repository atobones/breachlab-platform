...
export const PHANTOM_LEVEL_CONTENT: Record<number, PhantomLevelContent> = {
  ...
  24: { tier: "act4", goal: "Find the host-init marker process (argv contains phantom-host-init), read /opt/host-ns/host_secret via /proc/<pid>/root/, then run /opt/verify-pod-escape.sh.", realWorldSkill: "hostPID visibility + /proc/<pid>/root/ is the core of every pod-to-host escape in K8s.", mitigationVersion: "2026-04" },
